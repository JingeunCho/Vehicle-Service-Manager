...
- [x] 주변 주유소 찾기(Nearby Gas Station) UI/UX 구현 (Mock) 및 지도-표 실시간 동기화
- [x] 프리미엄 UI 컨트롤: Glassmorphism 커스텀 드롭다운, 이름/주소 검색, 유종별 필터링 레이아웃 최적화

### 🚧 진행 중 / 예정
- [ ] **[Phase 10]** 오피넷(Opinet) 실시간 유가 정보 연동 및 실제 네이버 지도 SDK 적용
- [ ] **[Phase 11]** 주변 EV 충전소 검색 (충전소 타입, 커넥터, 실시간 상태 필터)
- [ ] **[Phase 12]** 주변 정비소 검색 (경정비, 타이어 등 서비스 분야별 필터)
